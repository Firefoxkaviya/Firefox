...
async function startGojoMdNx() {
    ...
    GojoMdNx.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./Firefox")(GojoMdNx, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
Kon = await getBuffer(`https://hardianto.xyz/api/welcome3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/33d3f966368a28e47e183.png&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)

Tol = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/33d3f966368a28e47e183.png&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
                if (anu.action == 'add') {
